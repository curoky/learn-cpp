...
struct request_args {
  ...
  T next() {
    if (offset_ < tokens_.size()) {
      return parse<T>(tokens_[offset_++]);
    }
  ...
  T get(std::size_t index) {
    return parse<T>(tokens_[index + offset_]);
  }
...
FATAL_S(append, "append");
FATAL_S(at, "at");
FATAL_S(create, "create");
FATAL_S(get, "get");
FATAL_S(help, "help");
FATAL_S(insert, "insert");
FATAL_S(json, "json");
FATAL_S(list, "list");
FATAL_S(map, "map");
FATAL_S(size, "size");
FATAL_S(string, "string");
FATAL_S(substr, "substr");
...
using to_constructor_command =
    metadata::constructor_command<typename DataType::name, typename DataType::type,
                                  typename DataType::constructor::args>;
...
struct to_operation_command {
  ...
  using apply = metadata::operation_command<typename DataType::name, typename DataType::type,
                                            typename Operation::verb, typename Operation::method,
                                            typename Operation::result, typename Operation::args>;
};

template <typename DataType>
using to_operation_command_list =
    transform<typename DataType::operations, to_operation_command<DataType>>;
...
using known =
    list<data_type<str::list, std::vector<std::string>, constructor<>,
         ...
         data_type<str::string, std::string, constructor<std::string>,
                   operation<str::get, method::c_str::member_function, std::string()>,
                   operation<str::substr, method::substr::member_function,
                             std::string(std::size_t, std::size_t)>,
                   operation<str::append, method::append::member_function, void(std::string)>,
                   operation<str::size, method::size::member_function, std::size_t()>>,
         data_type<str::map, std::unordered_map<std::string, std::string>, constructor<>,
                   operation<str::get, method::operator_square_bracket::member_function,
                             std::string(std::string)>,
                   operation<str::insert, method::emplace::member_function,
...
struct ytse_jam {
  ...
  // TODO(curoky): unique BEFORE apply_to
  using result_t =
      apply_to<reject<transform<op_list, get_type::result>, curry<applier<std::is_same>, void>>,
               auto_variant>;
 ...
 private:
  ...
  static void call_ctor(index_sequence<Indexes...>, instance_t &instance, request_args &args) {
    instance.template emplace<T>(
        args.request_args::template get<at<ArgsList, Indexes>>(Indexes)...);
  }

  template <typename Method, typename Result, typename ArgsList, typename T, std::size_t... Indexes>
  static void call_method(index_sequence<Indexes...>, result_t &out, T &&instance,
                          request_args &args) {
    out.set_result_of([&]() {
      return static_cast<Result>(
          Method()(std::forward<T>(instance),
                   args.request_args::template get<at<ArgsList, Indexes>>(Indexes)...));
    });
  }
  ...
  struct command_parser {
    ...
    void operator()(tag<metadata::str::create>, instances_map &instances, request_args &args,
                    result_t &) const {
      // data_type_name -> ctor
      ...
      auto found = trie_find<transform<supported, get_type::name>>(
          ...
          [&](auto data_type_name) {  // tag<sequence>
            ...
            call_ctor<typename ctor::type, typename ctor::args>(arg_indexes(), instances[instance],
                                                                args);
          });
    ...
    void operator()(tag<metadata::str::help>, instances_map &, request_args &, result_t &) const {
      foreach
        <supported>([](auto data_type_tag) {  // indexed<data_type>
          using data_type = type_of<decltype(data_type_tag)>;
          if (decltype(data_type_tag)::value) {
            std::cout << std::endl;
          }
          std::cout << z_data<typename data_type::name>() << '(';
          foreach
            <typename data_type::constructor::args>([](auto arg_tag) {  // indexed<arg_type>
              using arg = type_of<decltype(arg_tag)>;
              if (decltype(arg_tag)::value) {
                std::cout << ", ";
              }
          ...
          foreach
            <typename data_type::operations>([](auto op_tag) {  // indexed<operation>
              using op = type_of<decltype(op_tag)>;
              std::cout << "- " << z_data<typename op::verb>() << '(';
              foreach
                <typename op::args>([](auto arg_tag) {  // indexed<arg_type>
                  using arg = type_of<decltype(arg_tag)>;
                  if (decltype(arg_tag)::value) {
                    std::cout << ", ";
                  }
    ...
    void operator()(tag<metadata::str::json>, instances_map &, request_args &, result_t &) const {
      std::cout << '{' << std::endl;
      foreach
        <supported>([](auto data_type_tag) {  // indexed<data_type>
          using data_type = type_of<decltype(data_type_tag)>;
          std::cout << "  \"" << z_data<typename data_type::name>() << "\": {" << std::endl;
          std::cout << "    \"type\": \"" << type_str<typename data_type::type>() << "\","
                    << std::endl;
          ...
          foreach
            <typename data_type::constructor::args>([](auto arg_tag) {  // indexed<arg_type>
              ...
              std::cout << "        \"" << decltype(arg_tag)::value << "\": \"" << type_str<arg>()
                        << '"';
              if (decltype(arg_tag)::value + 1 <
                  size<typename data_type::constructor::args>::value) {
                std::cout << ',';
              }
              std::cout << std::endl;
            });
          ...
          foreach
            <typename data_type::operations>([](auto op_tag) {  // indexed<operation>
              ...
              std::cout << "        \"result\": \"" << type_str<typename op::result>() << "\","
                        << std::endl;
              std::cout << "        \"args\": {" << std::endl;
              foreach
                <typename op::args>([](auto arg_tag) {  // indexed<arg_type>
                  using arg = type_of<decltype(arg_tag)>;
                  std::cout << "          \"" << decltype(arg_tag)::value << "\": \""
                            << type_str<arg>() << '"';
                  if (decltype(arg_tag)::value + 1 < size<typename op::args>::value) {
                    std::cout << ',';
                  }
                  std::cout << std::endl;
                });
              std::cout << "        }" << std::endl;  // args
              std::cout << "      }";                 // operation
              if (decltype(op_tag)::value + 1 < size<typename data_type::operations>::value) {
                std::cout << ',';
              }
              std::cout << std::endl;
            });
          ...
          std::cout << "  }";                 // data_type
          if (decltype(data_type_tag)::value + 1 < size<supported>::value) {
            std::cout << ',';
          }
          std::cout << std::endl;
        });
...
ytse_jam::result_t ytse_jam::handle(std::string const &command, request_args &args) {
  ...
  if (!trie_find<adjacent_unique<
          sort<cat<built_ins, transform<op_list, get_type::verb>>, sequence_compare<less>>>>(
